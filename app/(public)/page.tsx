...
export default function LandingPage() {
  return (
    <div className="flex flex-col min-h-screen">
      <header className="flex items-center justify-between px-16 py-6 bg-card border-b border-border">
        <div className="flex items-center gap-3">
          <HeartIcon />
          <span className="text-[26px] font-extrabold text-primary-dark">
            Care Companion
          </span>
        </div>
        <nav className="flex items-center gap-8">
          <a href="#" className="text-lg font-semibold no-underline text-ink">
            วิธีใช้งาน
          </a>
          <a href="#" className="text-lg font-semibold no-underline text-ink">
            สำหรับผู้ให้บริการ
      ...
      <section className="flex items-center gap-16 px-16 py-20 bg-gradient-to-b from-sky to-bg-cream">
        <div className="flex-1 flex flex-col gap-6">
          ...
          <h1 className="text-[52px] leading-[1.3] font-extrabold m-0 text-ink">
            เพื่อนเดินทาง
            <br />
            ที่ไว้ใจได้ ในทุกก้าวของคุณ
          </h1>
          <p className="text-xl leading-relaxed text-sub m-0 max-w-[560px]">
          ...
          <div className="flex gap-10 mt-4">
            {stats.map((s) => (
              <div key={s.label}>
                <div className="text-[28px] font-extrabold text-primary-dark">
                  {s.value}
                </div>
        ...
        <div className="flex-1 flex justify-center">
          <div className="w-[460px] h-[460px] rounded-[32px] bg-mint flex items-center justify-center overflow-hidden shadow-[0_20px_40px_rgba(47,122,104,0.15)]">
            <CompanionIllustration />
          </div>
        </div>
      </section>

      <section className="px-16 py-18 bg-card">
        ...
        <div className="flex gap-8 justify-center flex-wrap">
          {steps.map((step) => (
            <div
              key={step.n}
              className="flex-1 max-w-[340px] bg-bg-cream rounded-2xl p-8 flex flex-col gap-4"
            >
      ...
      <footer className="px-16 py-8 bg-primary-dark text-white flex justify-between items-center mt-auto">
